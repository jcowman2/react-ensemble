...
export const genAnimation = <State extends object>(
  ...
  defaults: State,
  fullConfig: Required<TrackConfig<State>>,
  rootLayer = "",
  startOffset = 0
) => {
  if (!track) {
  ...
  const calculatedLayers: Record<string, CalculatedTrackRegion<State>[]> = {};
  const activeVars = new Set<keyof State>();

  const tree = new IntervalTree<IntervalTreeSegment<State>>();
