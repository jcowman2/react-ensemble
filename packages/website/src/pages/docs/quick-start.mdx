...
`Timeline` uses this function under the hood, but you can also import it yourself. For more information, see the [TrackUtils.gen docs](/api/docs/TrackUtils#gen).
...
For example, our `multi` region is equivalent to this region:
...
For more information on layers, see the [Layers and Groups guide](/docs/layers-and-groups).
...
To make the square shift back into a circle, add another multi-region just after the first one:
